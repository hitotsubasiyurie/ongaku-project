...
def main():
    lprint(MESSAGE.WF_20251204_195021)

    file_str = easy_linput(MESSAGE.WF_20251204_195022, return_type=str)
    metadata_files = list(map(Path, [s.strip().strip("'\"") for s in file_str.split("|")]))

    src_ky = easy_linput(MESSAGE.WF_20251204_195023, default="", return_type=str)
    dst_ky = easy_linput(MESSAGE.WF_20251204_195024, default="", return_type=str)

    if not src_ky and not dst_ky:
        lprint(MESSAGE.WF_20251204_195025)
        return

    filter_catno = easy_linput(MESSAGE.WF_20251204_195026, default="N", return_type=str) == "Y"
    filter_trackcount = easy_linput(MESSAGE.WF_20251204_195027, default="Y", return_type=str) == "Y"
    ...
    # 用户编辑 merge_details
    lprint(MESSAGE.WF_20251204_195028.format(merge_details_file))
    if not easy_linput(MESSAGE.WF_20251204_195029, default="N", return_type=str)  == "Y":
        return

    # 应用 merge_details
    merge_details = list(rtoml.loads(merge_details_file.read_text(encoding="utf-8")).values())

    replace_mask = easy_linput(MESSAGE.WF_20251204_195030)
    if not len(replace_mask) == 4 and set(replace_mask).issubset({"0", "1"}):
        lprint(MESSAGE.WF_20251204_195031)
        return
    
    replace_blank = easy_linput(MESSAGE.WF_20251204_195032, default="Y", return_type=str)  == "Y"
    ...
    for row, col, d in zip(row_ind, col_ind, merge_details):
        ...
        if d[SRC_ALBUM] != src_unique_strs[row] or d[DST_ALBUM] != dst_unique_strs[col]:
            lprint(MESSAGE.WF_20251204_195033)
            return
    ...
    lprint(MESSAGE.WF_20251204_195034)
