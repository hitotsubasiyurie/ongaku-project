...
原始路径：例如 D:\\1.txt ，例如 D:\\download
目标路径：例如 D:\\2.txt ，例如 D:\\download_2
"""
    CGXLT9YQ = "请输入原始路径（文件或文件夹）：\n"
    P8C3P4XW = "请输入目标路径：\n"
    O5HIF3EV = "原始路径不存在或者目标路径已存在。"
    ...
    GB5JO189 = "重新编码文本文件"
    D1EG4CA9 = "请输入父目录路径：\n"
    DGO6VHRZ = "请输入要处理的文件后缀（默认为 .txt,.cue ）："
    JABXWHDS = """
a: 上一个编码 d: 下一个编码
w: 上一个文件 s: 下一个文件
p: 打开路径
...
"""
    ...
    VOLF5PUD = "生成合并日志"
    Q9YNQ293 = "请输入跳过关键字，links 中已含有该关键字的目标专辑将被跳过：\n"
    R3VY3KF6 = "请输入相似度门槛（推荐 90->80->75）（默认 90）："
    ...
    GCXAW6BC = "应用合并日志"
    DERFEKFV = "请输入合并日志路径：\n"
    J1H47YFK = "请输入元数据替换掩码 [catalognumber, date, album, tracks] （例如 0001）：\n"
    PLCIYBZW = "请输入是否允许自动替换目标空值（Y/N）（默认Y）："
    JNAIXTGI = "已应用合并日志。"
...
"""
    YHEH6TFR = "请输入保存目录路径，或者已有的元数据文件路径，将会生成或追加写入元数据文件：\n"
